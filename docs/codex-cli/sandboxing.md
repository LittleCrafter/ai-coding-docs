...
sandbox](./windows/windows-sandbox.md#windows-sandbox) when you run in PowerShell and the
...
[Config basics](./config-file/config-basic.md) explains how it works, and the
[Configuration reference](./config-file/config-reference.md) documents the exact keys for
...
policy](./agent-approvals-security.md#migrate-from-the-retired-untrusted-approval-policy).
...
- `auto_review`: eligible approval prompts go to a reviewer agent (see
  [automatic review](./sandboxing/auto-review.md)).
...
When a workflow needs a specific exception, use [rules](./agent-configuration/rules.md). Rules
...
entry points, see [Codex IDE extension settings](./developer-settings.md).
