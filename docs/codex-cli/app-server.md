...
`result.requirements` is `null` when no requirements are configured. See the docs on [`requirements.toml`](./config-file/config-reference.md#requirementstoml) for details on supported keys and values.
