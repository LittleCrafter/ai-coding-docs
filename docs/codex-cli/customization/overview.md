...
- **[Memories](./memories.md)** for useful context learned from prior work
- **Skills** for reusable workflows and domain expertise
- **[MCP](../extend/mcp.md)** for access to external tools and shared systems
- **[Subagents](../agent-configuration/subagents.md)** for delegating work to specialized subagents
...
[MCP](../extend/mcp.md) connects Codex to systems outside the local workspace.
...
- **Automate drift checks**: Use [scheduled tasks](../automations.md) to run recurring checks (for example, daily) that look for guidance gaps and suggest what to add to `AGENTS.md`.
...
connectors, package it as a [plugin](../build-plugins.md). Skills remain the
...
The skill directory can include a `scripts/` folder with CLI scripts that Codex invokes as part of the workflow (for example, seed data or run validations). When the workflow needs external systems (issue trackers, design tools, docs servers), pair the skill with [MCP](../extend/mcp.md).
...
[Model Context Protocol](../extend/mcp.md)
...
[Subagents](../agent-configuration/subagents.md)
...
If a skill depends on MCP, declare that dependency in `agents/openai.yaml` so Codex can install and wire it automatically (see [Build skills](../build-skills.md)).
...
2. Install a [plugin](../plugins.md) when a reusable workflow already exists. Otherwise, create a [skill](../build-skills.md) and package it as a plugin when you want to share it.
3. [MCP](../extend/mcp.md) when workflows need external systems (Linear, GitHub, docs servers, design tools).
4. [Subagents](../agent-configuration/subagents.md) when you're ready to delegate noisy or specialized tasks to subagents.