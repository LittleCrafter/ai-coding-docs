...
3. [Profile](./config-advanced.md#profiles) files selected with `--profile profile-name` (`~/.codex/profile-name.config.toml`)
...
Use that precedence to set shared defaults in `config.toml` and keep [profile files](./config-advanced.md#profiles) focused on the values that differ.
...
For one-off overrides via `-c`/`--config` (including TOML quoting rules), see [Advanced Config](./config-advanced.md#one-off-overrides-from-the-cli).
...
policy](./config-advanced.md#shell-environment-policy).
