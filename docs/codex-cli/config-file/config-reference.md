...
Use this page as a searchable reference for Codex configuration files. For conceptual guidance and examples, start with [Config basics](./config-basic.md) and [Advanced Config](./config-advanced.md).
...
keys in user-level config instead. Config [profile files](./config-advanced.md#profiles) live next to
...
| `desktop.custom_file_handlers.<id>` | `table` | User-level only. Defines an additional **Open in** target for the ChatGPT desktop app. See [Add custom file handlers](./config-advanced.md#add-custom-file-handlers) for examples and handler ID constraints. |
