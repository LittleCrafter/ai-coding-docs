...
3. Choose the replacement identity. For a durable non-human workflow on an
   ...
   account](./service-accounts.md). Otherwise, identify an
...
[Access tokens](./access-tokens.md#rotate-or-revoke-a-token).
...
[Roles and workspace permissions](./roles-and-workspace-permissions.md)
for the complete boundary model and [Plugin controls](./apps-and-connectors.md)
...
overview, see [Compliance API and audit events](./compliance-api.md).
