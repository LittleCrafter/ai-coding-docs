...
- Attach documents, spreadsheets, presentations, or PDF files when you want
  ChatGPT to summarize, compare, transform, or [create files for review](./artifacts-viewer.md).
- Add a screenshot, diagram, or other [image input](./image-inputs.md) when the
  task depends on visual context. Point out the area that matters instead of
  relying on the image alone.
- Ask ChatGPT to use [web search](./web-search.md) when the answer depends on
  current information, and ask for sources when you need to check the result.
- Use a [project](./projects.md) when related chats should share files,
...
[**Settings > General > Follow-up behavior**](./reference/settings.md#general).
...
[interactive shortcuts](./developer-commands.md#cli-interactive-shortcuts)
for details.
...
reliable, [schedule a task inside that chat](./automations.md#schedule-a-task-inside-a-chat).
...
investigate and propose an approach before editing. When [Goal mode](./long-running-work.md)
...
Codex runs local commands inside a [sandbox](./sandboxing.md)
