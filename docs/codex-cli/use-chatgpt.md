...
Work](./get-started-with-work.md) for example tasks, prompts, and best
practices.
...
- **Use your files and tools.** Bring in uploaded files,
  [projects](./projects.md), memories, ChatGPT Library, and installed
  [plugins](./plugins.md). Plugins can provide connected information, reusable
  workflows, and supported actions.
- **Create finished files.** Draft and refine [documents, presentations,
  spreadsheets, and PDF files](./artifacts-viewer.md). Review the result, ask
  for specific changes, and download the completed file.
- **Create visual and interactive work.** Generate or edit
  [images](./image-generation.md), make interactive
  [visualizations](./visualizations.md), and build or share websites and apps
  with [Sites](./sites.md).
- **Work across websites and apps.** Use the [browser](./browser.md) to
  research and interact with websites. In the desktop app, use the
  [browser extension](./chrome-extension.md),
  [Computer Use](./computer-use.md), and [appshots](./appshots.md) when
  those features are available.
- **Run code and review technical work.** Run code and shell commands, analyze
  data, inspect files, [review code](./code-review.md), and work with
  repositories your selected environment can access.
- **Delegate and continue longer tasks.** Split independent work across
  [subagents](./agent-configuration/subagents.md), follow their progress, and
  keep [long-running work](./long-running-work.md) active.
- **Repeat useful workflows.** Set up [scheduled
  tasks](./automations.md) for recurring work and use
  [skills](./skills-and-plugins.md) to reuse a workflow.
- **Talk through a task.** On supported plans in the desktop app, use
  [ChatGPT Voice](./features/voice.md) to start work, check progress, or
  change direction.
...
Learn more about [prompting](./prompting.md), or take the
...
[working with files](./artifacts-viewer.md).
