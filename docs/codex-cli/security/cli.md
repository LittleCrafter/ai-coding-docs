...
prerequisites](./cli/reference.md#authentication-and-prerequisites).
...
permissions](./cli/reference.md#local-scan-permissions).
...
The [CLI reference](./cli/reference.md#scan-artifacts) describes
...
[`validate` and `patch` reference](./cli/reference.md#codex-security-validate-and-codex-security-patch).
...
- [Run bulk security scans](./cli/bulk-scans.md) to discover GitHub
  repositories or scan a pinned CSV inventory.
- [Read the CLI FAQ](./cli/faq.md) for answers about scan history,
  false-positive feedback, coverage, and fix verification.
- [Run scans in CI](./cli/ci.md) to review pull requests, preserve
  results, and set a severity policy.
- [Use the CLI reference](./cli/reference.md) to check every flag,
  output format, artifact, and exit code.
- [Integrate the TypeScript SDK](./sdk.md) to run scans from an
