...
scan](./deep-scans.md). Deep scans take longer and search
more extensively.
...
- **Code changes:** [Review code
  changes](./code-changes.md) when a pull request or commit
...
- **After a fix:** [Fix and verify the
  finding](./fix-findings.md). Confirm that the issue no
...
finding](./fix-findings.md) to generate and review one
