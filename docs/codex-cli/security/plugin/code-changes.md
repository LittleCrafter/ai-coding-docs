...
finding](./fix-findings.md) or [export and track
findings](./export-findings.md).

## Automate reviews in CI/CD

If you have access to the beta standalone CLI, see [Run Codex Security in
CI](../cli/ci.md) for structured JSON, a severity policy, and SARIF
...
[plugin changelog](./changelog.md) before you depend on a
...
mode](../../non-interactive-mode.md). If your organization permits the [Codex
GitHub Action](../../github-action.md), it can install the CLI at runtime, but
