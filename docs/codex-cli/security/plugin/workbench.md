...
Install and enable the [Codex Security plugin](../plugin.md), then
...
See [Run a security scan](./scans.md), [Run a deep security
scan](./deep-scans.md), or [Review code changes for
security](./code-changes.md) for details about each scan
type.
...
findings](./fix-findings.md) for the remediation workflow.
...
The **Findings** tab shows findings from saved Codex Security scans. Imported
  ...
  [backlog triage workflow](./triage-backlog.md).
