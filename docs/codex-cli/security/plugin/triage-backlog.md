...
Backlog triage starts from existing findings. To search the repository for new
  vulnerabilities, [run a security scan](./scans.md). Triage
...
- the [`$codex-security:fix-finding`](./fix-findings.md)
...
- `confirmed`: After a person accepts the finding for remediation, use
  [`$codex-security:fix-finding`](./fix-findings.md) to fix and
...
```
  ...
  policies](../../agent-approvals-security.md) in place.
