...
Start with a [standard scan](./scans.md) to check your scope
...
[plugin changelog](./changelog.md) for release details.
...
changelog](./changelog.md) before you update the plugin or
...
finding](./fix-findings.md).

To review a pull request, commit, branch range, or local patch, use [Review code
changes](./code-changes.md). A deep scan never substitutes
