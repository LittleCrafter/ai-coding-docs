...
See [Models](./models.md) for model selection, [Pricing](./pricing.md) for
...
[Pets controls](./pets.md) in the ChatGPT desktop app on macOS
...
[Appshots](./appshots.md) bring a screenshot and available text
...
[GPT-6 Astra](./models.md#gpt-6-astra) combines advanced reasoning, computer
...
See [usage and pricing](./pricing.md) before starting a large task.
...
- **Use your browser:** Work in [Edge, Brave, Opera, or Vivaldi](./chrome-extension.md)
...
- **Use a website's tools:** With [Site tools (WebMCP)](./webmcp.md), ChatGPT
...
- **Sign in through the cloud browser:** On eligible plans, continue a task
  ...
  Follow the [sign-in request](./browser.md#web-sign-in-to-a-website)
...
[Scheduled tasks](./automations.md#web-trigger-tasks-from-app-events) can now
...
- **Apple Messages:** [Find chats, summarize messages, prepare replies, and send through Messages on your Mac](./plugins.md#app-use-apple-messages-from-codex). The plugin is available on all plans in the ChatGPT desktop app for macOS. Use it in ChatGPT Work and Codex, not in regular ChatGPT chats. By default, ChatGPT sends messages only after you approve the message and its recipients.

- **Site co-editing:** Where available, [invite active members of your workspace as editors](./sites.md#collaborate-on-a-site). Editors can refine the Site and publish updates after its owner publishes it for the first time. Invited editors can read the Site's live database data; owners retain control of sharing and settings.

- **Editable Site URLs:** Where available, [choose a new ChatGPT-hosted address for an existing Site](./sites.md#change-a-site-url) without redeploying it. The previous address redirects to the new one.

- **Computer History in Europe:** Use [Computer History](./customization/computer-history.md) in the EEA, Switzerland, and the United Kingdom. It remains off by default for ChatGPT Pro, Business, and Enterprise users on macOS. Business and Enterprise administrators must enable access first.

- **Shared thread snapshots:** [Share a read-only snapshot of a local Codex thread](./use-chatgpt.md#share-a-read-only-snapshot-of-a-codex-thread) from the ChatGPT desktop app for macOS. Personal-account links are viewable by anyone with the link; workspace-account links are limited to the originating workspace. Codex redacts known secret patterns, but review the snapshot before sharing because sensitive content may remain.

- **Unified pinned threads:** Keep your [pinned chats](./projects.md#app-organize-projects-and-chats) in sync between desktop and iOS.
...
[GitLab support](./third-party/gitlab.md) is available in beta on all ChatGPT
...
[Computer History](./customization/computer-history.md) turns activity across
...
The [ChatGPT desktop app for Linux](./linux/linux-app.md) is now available in
...
work](./import.md) from **Claude Code**, **Claude Cowork**, or
...
Cyber](./cyber-safety.md#trusted-access-for-cyber) and applies only to the
...
[ChatGPT Voice](./features/voice.md) now supports uploaded files and
[ChatGPT Projects](./projects.md). Ask questions about a document during a
...
Three new [plugins](./plugins.md) bring classroom-specific workflows to
...
share the same [usage limits and credits](./pricing.md).
...
[import Cursor and Claude Code setup](./import.md) without duplicating
synced conversations.

Use `--approve-for-me` to enable [automatic approval
review](./sandboxing/auto-review.md) for eligible requests without expanding
...
Use the [Codex Security workbench](./security/plugin/workbench.md) to review
scan progress and findings, or [configure a deep
scan](./security/plugin/deep-scans.md) when you need a more thorough
assessment. Check the [plugin changelog](./security/plugin/changelog.md) to
confirm which features your installed version supports.

### Review GitHub pull requests for security risks

[Codex Security Review](./security/security-review.md) analyzes pull-request
...
[usage limits and rates](./pricing.md) make Terra a stronger fit for everyday
...
In the ChatGPT desktop app, the [built-in browser](./browser.md) can find
...
The [Chrome extension](./chrome-extension.md) lets you mention open tabs,
...
folder](./projects.md#use-local-projects-for-folders-and-codebases), the
...
Learn more about [image generation](./image-generation.md).
...
workbench](./security/plugin/workbench.md) to review saved scans, findings,
...
changelog](./security/plugin/changelog.md) before relying on a new feature.
...
package to [run scans from the CLI](./security/cli.md), review pull-request
changes and upload SARIF results in [CI](./security/cli/ci.md), or run
resumable [bulk scans](./security/cli/bulk-scans.md) across GitHub
repositories or a pinned CSV inventory.

The [Codex Security TypeScript SDK](./security/sdk.md) also lets you build
...
For custom clients, the [app server](./app-server.md) can filter pinned
...
[app-server security requirements](./app-server.md#connect-the-cli-terminal-ui)
...
[GPT-5.6 Sol](./models.md#recommended-models) now powers Codex cloud code
...
affected. Review the [deprecated Codex models](./models.md#deprecated-codex-models)
and [workspace model
availability](./enterprise/workspace-model-availability.md) before the
cutoff.
...
[ChatGPT Voice](./features/voice.md), powered by GPT-Live, lets you talk
...
On macOS, say, “Take a look at this” to share an [appshot](./appshots.md) of
your frontmost window when **Screen context** is on.

Voice is available with Plus, Pro, Business, Edu, and Enterprise plans in the
desktop app and through [Remote on iOS](./remote-connections.md#set-up-mobile-access).
...
folder](./projects.md#use-local-projects-for-folders-and-codebases).
...
desktop](./use-chatgpt.md#compare-chatgpt-work-and-codex-on-desktop) to choose the
...
[Codex Micro](./features/codex-micro.md), a limited-run physical control
...
[`amazon-bedrock` provider](./amazon-bedrock.md) with a Bedrock API key or the
...
[ChatGPT Work](./get-started-with-work.md) in ChatGPT can gather context from
your files and [plugins](./plugins.md),
...
[GPT-5.6](./models.md), it can break a goal into steps and work for hours while
you follow its progress, answer questions, change direction, and approve
important actions.

[Scheduled tasks](./automations.md) can keep that work moving when you're away
...
The [GPT-5.6 family](./models.md#recommended-models) offers three recommended
...
[ChatGPT desktop app](./app.md) for macOS and Windows. Codex keeps its
...
[Computer Use](./computer-use.md) powered by GPT-5.6, and multi-repository
projects.
...
[Record & Replay](./extend/record-and-replay.md) lets you show ChatGPT or
...
[Chat handoff](./remote-connections.md#hand-off-a-chat-between-hosts)
...
[Developer mode](./browser.md#app-developer-mode) gives Codex controlled
...
[Sites](./sites.md) lets ChatGPT create, save, deploy, and inspect websites,
...
You can [use Codex with Amazon Bedrock](./amazon-bedrock.md) for local
...
[Computer use](./computer-use.md#windows-foreground-use) added support for
...
[Appshots](./appshots.md) send the frontmost app window to Codex with a
...
[Goal mode](./prompting.md#goal-mode) left experimental status and is
available in the Codex app, IDE extension, and CLI for objectives that can take
hours or days. [Locked use](./computer-use.md#locked-use) lets Codex
...
continue from your phone. See [Remote connections](./remote-connections.md)
...
[Codex access tokens](./enterprise/access-tokens.md) for trusted scripts,
...
The [Chrome extension](./chrome-extension.md) can work in
...
members to create [Codex access tokens](./enterprise/access-tokens.md) for
...
[GPT-5.5](./models.md) arrived in Codex as the recommended model for most
...
[Computer Use in the built-in browser](./browser.md#app-computer-use-in-the-browser)
...
through [automatic approval review](./sandboxing/auto-review.md),
...
The [built-in browser](./browser.md) added live previews and page
comments, while [Computer Use](./computer-use.md) let Codex see and
...
[Standalone chats](./projects.md#start-without-a-project) made it
possible to begin without choosing a project folder. The same release added
[scheduled tasks inside a chat](./automations.md#schedule-a-task-inside-a-chat),
pull-request context, richer file previews, and [Memories](./customization/memories.md) for
...
learn how to [review changes in the app](./code-review.md).

## March 23–27, 2026

### Package workflows as plugins

[Plugins](./plugins.md) launched as installable bundles of skills,
...
[Scheduled tasks](./automations.md) could run locally or in a worktree
...
Codex also learned to read the [integrated terminal](./integrated-terminal.md#run-and-validate-your-project)
...
[Local and Worktree handoff](./environments/git-worktrees.md#working-between-local-and-worktree)
...
became the foundation for [steering and queuing](./prompting.md#steering-and-queuing)
