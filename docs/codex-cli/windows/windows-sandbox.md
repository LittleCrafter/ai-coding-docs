...
Use Codex on Windows with the native [ChatGPT desktop app](./windows-app.md), the
[CLI](../codex/cli.md), or the [IDE extension](../codex/ide.md).
...
>
> Work across projects, run parallel chats, and review results in one place with the native Windows app.
...
Running Codex in full access mode means Codex is not limited to your project
  ...
  [rules](../agent-configuration/rules.md) for specific exceptions, or set your
  [approval policy to
...
Use the native Windows sandbox by default. Choose [WSL](./wsl.md)
