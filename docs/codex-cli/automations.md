...
affected. See [GPT-5.5 retirement](./models.md#gpt-55-retirement) for
...
project or on a new [worktree](./environments/git-worktrees.md). Both options run in the
...
on a dedicated background [worktree](./environments/git-worktrees.md). Use
...
[skills](./build-skills.md) to define the action and provide tools and context.
...
[sandboxing overview](./sandboxing.md). For filesystem and network
rules, see [Permissions](./permissions.md).
...
- If your sandbox mode is **workspace-write**, tool calls fail if they require
  ...
  sandbox using [rules](./agent-configuration/rules.md).
- If your sandbox mode is **full access**, background scheduled tasks carry
  ...
  using [rules](./agent-configuration/rules.md) to selectively define which commands the agent
...
[Admin-enforced requirements (`requirements.toml`)](./enterprise/managed-configuration.md#admin-enforced-requirements-requirementstoml).
...
Create a new skill that tries to fix a bug introduced by your own commits by creating a new `$recent-code-bugfix` and [store it in your personal skills](./build-skills.md#where-to-save-skills).
