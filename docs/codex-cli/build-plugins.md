...
Custom GPTs in ChatGPT Enterprise are being retired. If you use a custom GPT, follow [Moving your custom GPT workflows to plugins](./migrate-custom-gpts.md) to migrate, test, and share its replacement. Use the steps below to create a new plugin from scratch or edit an existing one.
...
[Apps](https://help.openai.com/en/articles/11487775) connect ChatGPT to other services. [Skills](./skills-and-plugins.md#use-skills-for-repeatable-work) provide instructions for a task, such as summarizing a document in a consistent format.
...
If the plugin needs an app connection, follow the prompts to connect it. To find and install a plugin, see [Plugins](./plugins.md).
...
Note the app’s name and what it supports, then tell Plugin Creator how you want to use it. For more on browsing and setup, see [Plugins](./plugins.md#use-and-install-plugins).
...
Include a sample prompt with the plugin link and let teammates know who maintains it. See [Plugins](./plugins.md) for more on installation and use.
...
If a teammate can open the plugin but cannot use it, check plugin installation, workspace permission to use plugins, and access to required apps separately. Admins can review [Plugin controls](./enterprise/apps-and-connectors.md).
...
For workspace distribution through GitHub, see [Plugin management](./enterprise/plugin-management.md).