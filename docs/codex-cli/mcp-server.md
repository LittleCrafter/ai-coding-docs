...
Use the [Codex app server](./app-server.md) for integrations that need
authentication, conversation history, approvals, and streamed agent events.

The app server uses its own [JSON-RPC protocol](./app-server.md#protocol). It
...
Codex continues to support [external MCP servers](./extend/mcp.md).
