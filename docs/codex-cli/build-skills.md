...
[Record & Replay](./extend/record-and-replay.md). The recorder captures the
...
Add `agents/openai.yaml` to configure UI metadata in the [ChatGPT desktop app](./app.md), to set invocation policy, and to declare tool dependencies for a more seamless experience with using the skill.
