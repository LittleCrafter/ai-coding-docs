...
A project is one ongoing conversation where Claude coordinates a stream of related work for you. You tell it what needs doing and it starts a thread for each task. Each thread is a [cloud session](./claude-code-on-the-web.md): Claude Code running in the cloud rather than on your machine. Threads run in parallel and keep going after you close the laptop.
...
<img src="media/images/claude-projects-overview.svg" alt="Diagram of a project. You write in the project conversation, where Claude answers or starts a thread. Each thread is a cloud session working on its own branch and pull request. The Overview pane lists threads by state, such as ready for review, waiting on you, and working." width="600" height="250" />

<img src="media/images/claude-projects-overview-dark.svg" alt="Diagram of a project. You write in the project conversation, where Claude answers or starts a thread. Each thread is a cloud session working on its own branch and pull request. The Overview pane lists threads by state, such as ready for review, waiting on you, and working." width="600" height="250" />
...
2. **Install the Claude GitHub App on the project's repositories**
...
3. **Authorize SSO for organizations that enforce it**
...
* Projects are available at claude.ai/code and in the desktop app, not in the terminal CLI or through Amazon Bedrock, Google Cloud's Agent Platform, or Microsoft Foundry. The CLI's [`claude project`](./cli-reference.md) command, which manages local Claude Code state for a directory, is unrelated.
