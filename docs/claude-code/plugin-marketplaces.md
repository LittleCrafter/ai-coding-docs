...
* <a id="reserved-name-spellings"></a>Another spelling of a reserved name: one that differs only by a trailing dot or by a symbol other than an underscore in place of a hyphen, so `claude.code.plugins` counts as `claude-code-plugins`. This check requires Claude Code v2.1.280 or later.
* `npm`, `pip`, `uv`, `cargo`, `github`, or `gh`, in any casing. This check requires Claude Code v2.1.275 or later.

A third-party marketplace already registered under a reserved name or another spelling of one also stops loading. With a reserved name, you see the [registered from an untrusted source](./errors.md#marketplace-is-registered-from-an-untrusted-source) error. With another spelling of one, `/plugin`, `claude plugin install`, and `claude plugin update` warn that [its entry is ignored](./errors.md#marketplace-name-is-another-spelling-of-a-reserved-name). To fix either, remove the marketplace, then re-add it under a different name or from the official Anthropic source.
