...
export const BackToIndex = ({href = '#all-settings', label = 'Back to index'}) => {
  ...
  return <div>
      ...
      `}</style>
      <a href={href} aria-hidden={!show} tabIndex={show ? 0 : -1}>
...
export const ReferenceFilter = ({placeholder, noun, facets, facetOrder, columnHelp, children}) => {
  ...
  return <>
      ...
      <div ref={rootRef}>
        <div>
        <div>
          <input ref={searchRef} value={q} onChange={e => onChange(e.target.value)} placeholder={placeholderText} aria-label={placeholderText} />
    ...
    return <div key={f.name} ref={el => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (!isOpen) setMenu(f.name); else focusSelected(f.name);
            ...
            {cur !== 'All' && <button type="button" onClick={() => setFacet(f.name, 'All')} aria-label={'Clear ' + f.label + ' filter'} title={'Clear ' + f.label + ' filter'}>
                ×
              </button>}
      ...
      return <button key={c} role="option" aria-selected={selected} tabIndex={-1} onClick={() => setFacet(f.name, c)}>
                      <span aria-hidden="true">
                        {selected ? '✓' : ''}
                      </span>
                      {optionLabel(f, c)}
        ...
        {sortOptions.length > 1 && <div role="group" aria-label="Sort by">
            <span>Sort by</span>
            {sortOptions.map(o => {
    const on = sortBy === o;
...
<a id="modelpicker-options"></a>

<a id="modelpicker-replacebuiltinoptions"></a>
...
<a id="modelpricing-multiplier"></a>

<a id="modelpricing-overrides"></a>
...
Claude Code uses a row's rates exactly as you wrote them, without adding the fast-mode surcharge or the [US-only-inference rate](https://platform.claude.com/docs/en/about-claude/pricing). If you also set `multiplier`, Claude Code applies it on top of the row's rates. Claude Code drops a row with a rate it can't parse, or a `multiplier` it can't parse, and keeps the rest; see [Fix a broken settings file](./settings.md#fix-a-broken-settings-file).
...
<a id="exclude-sensitive-files"></a>
...
<a id="sandbox-credentials-files-extract"></a>

<a id="sandbox-credentials-files-onextractnomatch"></a>

<a id="sandbox-credentials-files-decode"></a>

<a id="sandbox-credentials-files-maskclaims"></a>

<a id="sandbox-credentials-files-maskduplicates"></a>

<a id="sandbox-credentials-files-injecthosts"></a>
...
<a id="sandbox-credentials-envvars-extract"></a>

<a id="sandbox-credentials-envvars-onextractnomatch"></a>

<a id="sandbox-credentials-envvars-decode"></a>

<a id="sandbox-credentials-envvars-maskclaims"></a>

<a id="sandbox-credentials-envvars-injecthosts"></a>
...
<a id="file-suggestion-settings"></a>
...
<a id="hook-configuration"></a>

<a id="hook-and-skill-settings"></a>
...
If you set this key in the [claude.ai admin console](./server-managed-settings.md), claude.ai also applies it when anyone in your organization adds a marketplace from a git repository on claude.ai, as [How restrictions work](./plugin-marketplaces.md#how-restrictions-work) describes.
...
<a id="authentication-and-login"></a>
...
<a id="compute-managed-settings-with-a-policy-helper"></a>
