...
* **Teammates**: this session's own [agent team](./agent-teams.md) teammates.
...
* **Your [cloud sessions](./claude-code-on-the-web.md)**: shown while this session is connected to [Remote Control](./remote-control.md).
...
A session inside a container and a session on the host can't reach each other. Two sessions inside the same container can still message each other, including on a [self-hosted runner](./self-hosted-environments.md). A session inside WSL 2 and a native Windows session on the same computer can't reach each other either.
...
<a id="own-child-messages"></a>Claude Code runs messages arriving on the socket through the same [inbound controls](#control-inbound-messages) as any other peer message, with one exception and one prerequisite:
...
* [Channels](./channels.md): push external events, such as CI results or chat messages, into a session
